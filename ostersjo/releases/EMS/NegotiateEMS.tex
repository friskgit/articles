...
\lhead{\small{}}
\chead{\small{EMS: Electroacoustic Music Studies Network - Beijing 2006}\vspace{0.2cm}}
\rhead{\small{}}
...
\title{EMS - Beijing 2006\\ \vspace{1cm}
Negotiating the Musical Work. \\An empirical study on the
...
Music - Lund University}\\{\small henrik.frisk@mhm.lu.se, stefan\_ostersjo@hotmail.com}}
...
into `composer' and `performer' \cite{wis96}. This split calls for an
...
% In the discussion on musical works,
% the distinction between art works made by Nelson Goodman is approached.
% Works depending on notation (like literature, choreography, music
% specified in scores) are allographic and works where artists produce
% objects are autographic. Autographic works can be falsified: In fact,
% even the most precise duplication always results in a false
% copy\footnote{For a recent discussion on the possibility to forge art
%   works with reference to Goodman's distinction, see the debate
%   between Peter Kivy and Kirk Pillow in The Journal of Aesthetics and
%   Art Criticism, Vol. 60, No. 2, Spring, 2002}. Allographic works
% can have many instances, deriving their mutual identity from the
% notation.

% Finally, we find that the ontology of the mixed media work (in lack of a general
...
%  notation in this discussion and that the electronic part is itself another
...
Figure \ref{cons-rep}).
 ...
 \begin{center}
  ...
  \caption{\small{\emph{Within the Western art music tradition the score is commonly
 ...
 writing \cite{ric91}. When writing takes the place of dialogue, the
 immediate face-to-face communication is replaced by inscription and
 the semantic autonomy of the text. The disconnection between
 the author's intention and the meaning of the text is a key issue in
...
that, in one move, poses the whole series of problems that we are now
going to confront concerning the relations between explanation and
interpretation. These relations arise at the time of
reading. \cite[pp. 109-10]{ric91}
\end{quote}
...
notating it. In the case of a written text, the intention of the author is
not equal to the meaning of the text. The author is present in the
text, but only as a first reader. Similarly, this suggests that the
construction of a score-based work consists of dialectic interplay
...
\begin{figure}[!h]
  \begin{center}
    ...
    \caption{\small{\emph{In the light of Ric{\oe}ur we arrive at a modified scheme
...
Horacio Vaggione \cite{vaggione01}. The composer always has to
...
\begin{figure}[!h]
  \begin{center}
    ...
    \caption{\small{\emph{Our schematic model of the interaction between
...
hides...'' \cite[p. 144]{adorno81} A crucial fact about musical works
...
the compositional process by the creative `Subject' \cite{pad91}. The
...
ear of the experienced listener. \cite[p. 29]{meyer73}
\end{quotation}
...
have, or about the work's significance. \cite[p. 82]{stecker}
\end{quotation}
...
\cite[38-9]{levinson}. In order to take a position in cases where a
...
interpretation indicates or details such a conception. \cite[pp. 38-9]{levinson}
\end{quotation}
...
add features that the work leaves vague or undetermined. \cite[p. 80]{stecker}
\end{quotation}
...
a language that signifies itself \cite[pp. 30]{nat89}. Jean Molino
...
impossibility of exploiting it'' \cite[pp. 126-7]{molino}. Molino aims
...
Eco calls the ``cultural convention''. \cite[pp. 61-6]{eco71}
...
human communication as a total social fact.'' \cite{molino} Music,
...
\cite[pp. 34]{nattiez}. Molino argues for a three level symbolic
analysis; ``the poietic, the esthesic and the `neutral' analysis of
the object'' \cite{molino}. Three modes of analysis all representing
...
performance. \cite[pp. 92]{nattiez}
\end{quotation}

Leaving the problematic concept of the neutral level aside\footnote{It
  has been extensively debated elsewhere, see footnote 8 of \cite[p.
...
relevant'', that which one hears \cite[pp. 140-3]{nattiez}. The three
...
except for them. \cite[pp. 12-3]{nattiez}
\end{quotation}
...
and the esthesic begins in score-based music (\emph{ibid}, pp. 72).
...
passing through its `writing down', to its performance'' \cite[pp.
...
drawn\footnote{For an in depth description of the empirical studies
  performed see \cite{frisk-ost06}.}:
\begin{enumerate}
...
pointing at the code used to encode the message \cite[pp.
